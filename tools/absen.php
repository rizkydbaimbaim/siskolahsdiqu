...
if ($iduser !== NULL && $_GET['api_key'] == KEY) {
    ...
    $iduser = $idusers[0];
    $level  = end($idusers);
    $user   = ($level != 'siswa') ? mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM siswa WHERE id_siswa=". $iduser)) : mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM siswa WHERE id_siswa=". $iduser));
    ...
    switch ($action) {
        ...
        case 'absen':
            ...
            if ($exec > 0) {
                ...
                mysqli_query($koneksi,"INSERT INTO absen_daring(idsiswa,tanggal,jam,ket) VALUES(". $iduser .", '". date('Y-m-d', time()) ."', '". date('H:i:s', time()) ."','H')");
