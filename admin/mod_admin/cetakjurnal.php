...
$jurnal = fetch($koneksi, 'jurnal', ['kelas' => $kelas, 'mapel' => $mapel, 'guru' =>$guru]);
$absen = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absen_mapel WHERE tgl_absen='$jurnal[tanggal]' 
...
<body style="font-size: 12px;">
  ...
  <center><h3><?= strtoupper($setting['sekolah']) ?></h3></center>
   ...
   <div style="padding-left:20px;margin-right:50px ;" class="col-md-12">
	...
	<th width='8%' style="text-align: center;">Kehadiran Siswa</th>
	</tr>
	 </thead>
	 <?php
	  $query = mysqli_query($koneksi, "select * from jurnal 
	 JOIN m_hari ON m_hari.inggris=jurnal.harix WHERE mapel='$mapel' AND kelas='$kelas' AND guru='$guru' ORDER BY jurnal.id ASC");
      $no = 0;
