...
if ($pg == 'tambah') {
    ...
    $cek = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM mapel_kisi WHERE  kode='$kode'"));
    ...
    if ($pengawas['level'] == 'admin') {
        ...
        if ($cek > 0) :
            ...
            $exec = mysqli_query($koneksi, "INSERT INTO mapel_kisi (kode, idpk, nama,level,status,idguru,kelas) VALUES ('$kode','$id_pk','$nama','$level','$status','$guru','$kelas')");
...
if ($pg == 'hapus') {
    $kode = $_POST['kode'];
	$edit2=$koneksi->query("select * from mapel_kisi WHERE id_mapel='$kode'");
  $ms=mysqli_fetch_array($edit2);
    ...
    mysqli_query($koneksi, "DELETE FROM kisi WHERE id_mapel='$ms[kode]' ");
	  mysqli_query($koneksi, "DELETE FROM kisi2 WHERE id_mapel='$ms[kode]' ");
    $exec = mysqli_query($koneksi, "DELETE FROM mapel_kisi  WHERE id_mapel in (" . $kode . ")");
    if ($exec) {
        echo 1;
    } else {
        echo 0;
