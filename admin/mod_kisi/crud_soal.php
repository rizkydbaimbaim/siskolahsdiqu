...
if ($pg == 'import_candy') {
    if (isset($_FILES['file']['name'])) {
        ...
        if ($ext <> 'xls') {
            ...
            $sukses = $gagal = 0;
            $exec = mysqli_query($koneksi, "DELETE FROM kisi WHERE id_mapel='$id_mapel' ");
			$exec = mysqli_query($koneksi, "DELETE FROM kisi2 WHERE id_mapel='$id_mapel' ");
            for ($i = 5; $i <= $hasildata; $i++) :
                $no = $data->val($i, 0);
                $nokd = addslashes($data->val($i, 1));
                $kd = addslashes($data->val($i, 2));
                $materi = addslashes($data->val($i, 3));
				$nkd = addslashes($data->val($i, 4));
				$indikator = addslashes($data->val($i, 5));
				$kog1 = addslashes($data->val($i, 6));
				$kog2 = addslashes($data->val($i, 7));
				$kog3 = addslashes($data->val($i, 8));
				$jenis = addslashes($data->val($i, 9));
				 $nosoal = addslashes($data->val($i, 10));
				$skor = addslashes($data->val($i, 11));
                $id_mapel = $_POST['id_mapel'];
                $exec = mysqli_query($koneksi, "INSERT INTO kisi2 (id_mapel,nkd,indikator,kog1,kog2,kog3,jenis,nosoal,skor) VALUES ('$id_mapel','$nkd','$indikator','$kog1','$kog2','$kog3','$jenis','$nosoal','$skor')");

                if ($nokd <> '' and $kd <> '') {
                    $exec = mysqli_query($koneksi, "INSERT INTO kisi (id_mapel,nokd,kd,materi) VALUES ('$id_mapel','$nokd','$kd','$materi')");
