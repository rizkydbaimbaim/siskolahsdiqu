...
<head>
    ...
    <title>Nilai_KI-3_<?= $mapel ?>_Kelas_<?= $kelas ?></title>
...
<body style="font-size: 12px;">
   ...
   <center><h5>NILAI PENGETAHUAN (KI-3)</h5></center>
   ...
   <div style="padding-left:20px;margin-right:50px ;" class="col-md-12">
	...
	<th style="text-align: center;">Nama Siswa</th>
	 <?php
			$querys = mysqli_query($koneksi,"select * from nilai_ph WHERE kelas='$kelas' AND mapel='$kodemap' AND nilai >0 GROUP BY ket ORDER BY id");
     ...
     <tbody>
	        ...
	        <?php
			$queryxx = mysqli_query($koneksi,"select * from nilai_ph
     	...
     	<td style="text-align: center;"><?= round($nilai['nilai']) ?></td>
			 <?php } ?>
			  ...
			  <?php
			$queryxr = mysqli_query($koneksi,"select AVG(nilai) AS rata from nilai_ph
