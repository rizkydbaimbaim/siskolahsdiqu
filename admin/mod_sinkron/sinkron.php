                ...
                if ($r <> null) {
                    ...
                    foreach ($r['siswa'] as $r) {
                        $sql = mysqli_query($koneksi, "insert into siswa
                            (id_siswa,id_kelas,idpk,nis,no_peserta,nama,level,ruang,sesi,username,password,foto,server,agama,npsn) values 			
                            ('$r[id_siswa]','$r[id_kelas]','$r[idpk]','$r[nis]','$r[no_peserta]','" . addslashes($r['nama']) . "','$r[level]','$r[ruang]','$r[sesi]','$r[username]','$r[password]','$r[foto]','$r[server]','$r[agama]','$r[npsn]' )");
                ...
                if ($sync <> null) {
                    ...
                    foreach ($sync['bank'] as $banksoal) {
                        ...
                        if ($banksoal['status'] == 1) {
                            $sql = mysqli_query($koneksi, "insert into mapel (id_mapel,kode,idpk,idguru,nama,jml_soal,jml_esai,jml_multi,jml_bs,jml_urut,tampil_pg,tampil_esai,tampil_multi,tampil_bs,tampil_urut,opsi,level,groupsoal,bobot_pg,bobot_esai,bobot_multi,bobot_bs,bobot_urut,kelas,date,status,sts) 
			values ('$banksoal[id_mapel]','$banksoal[kode]','$banksoal[idpk]','$banksoal[idguru]','$banksoal[nama]','$banksoal[jml_soal]','$banksoal[jml_esai]','$banksoal[jml_multi]','$banksoal[jml_bs]','$banksoal[jml_urut]','$banksoal[tampil_pg]',
			'$banksoal[tampil_esai]','$banksoal[tampil_multi]','$banksoal[tampil_bs]','$banksoal[tampil_urut]','$banksoal[opsi]','$banksoal[level]','$banksoal[groupsoal]','$banksoal[bobot_pg]','$banksoal[bobot_esai]','$banksoal[bobot_multi]','$banksoal[bobot_bs]','$banksoal[bobot_urut]','$banksoal[kelas]','$banksoal[date]','$banksoal[status]','$banksoal[sts]')");
                            if (!$sql) {
                                $gagal2++;
                            } else {
                                $masuk2++;
                    ...
                    (id_soal,id_mapel,nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,file,file1,fileA,fileB,fileC,fileD,fileE,ket,sts) values 			
                    ('$soal[id_soal]','$soal[id_mapel]','$soal[nomor]','$soalx','$soal[jenis]','$pilA','$pilB','$pilC','$pilD','$pilE','$soal[jawaban]','$soal[file]','$soal[file1]','$soal[fileA]','$soal[fileB]','$soal[fileC]','$soal[fileD]','$soal[fileE]','$soal[ket]','$soal[sts]')");
                                    if (!$sqlsoal) {
                                        $gagal3++;
                                    } else {
                                        $masuk3++;
...
(id_ujian,kode_nama,id_pk,id_guru,id_mapel,kode_ujian,nama,groupsoal,jml_soal,jml_esai,jml_multi,jml_bs,jml_urut,opsi,tampil_bs,tampil_urut,
tampil_pg,tampil_esai,tampil_multi,lama_ujian,tgl_ujian,tgl_selesai,waktu_ujian,selesai_ujian,level,kelas,sesi,acak,token,status,hasil,kkm,ulang,soal_agama,reset) values 
('$jadwal[id_ujian]','$jadwal[id_pk]','$jadwal[id_guru]','$jadwal[id_mapel]','$jadwal[kode_ujian]','$jadwal[nama]','$jadwal[groupsoal]','$jadwal[jml_soal]','$jadwal[jml_esai]','$jadwal[jml_multi]','$jadwal[jml_bs]','$jadwal[jml_urut]',
'$jadwal[opsi]','$jadwal[tampil_bs]','$jadwal[tampil_urut]','$jadwal[tampil_pg]','$jadwal[tampil_esai]','$jadwal[tampil_multi]','$jadwal[lama_ujian]','$jadwal[tgl_ujian]','$jadwal[tgl_selesai]',
 '$jadwal[waktu_ujian]','$jadwal[selesai_ujian]','$jadwal[level]','$jadwal[kelas]','$jadwal[sesi]','$jadwal[acak]','$jadwal[token]','$jadwal[status]','$jadwal[hasil]','$jadwal[kkm]','$jadwal[ulang]','$jadwal[soal_agama]','$jadwal[reset]')");
                        if (!$sqljadwal) {
                            $gagal4++;
                        } else {
                            $masuk4++;
