...
$query = "SELECT max(id_surat) as maxKode FROM surat_keluar WHERE month(tgl_surat)='$bulan'";
...
?>
        ...
        <div class='col-md-12'>
            <div class='box box-solid'>
                <div class='box-header with-border '>
                    <h3 class='box-title'><i class="fas fa-book fa-fw   "></i>Input Surat Keluar</h3>
                    <div class='box-tools pull-right'>
				<a href="?pg=surat_keluar&ac=lihat" class="btn btn-sm btn-primary" ><i class='fa fa-search'></i> Buka Surat</button></a>
                    </div>
                </div><!-- /.box-header -->
                <div class='box-body'>
	...
	<?php } elseif ($ac == 'lihat') { ?>
        ...
        <div class='col-md-12'>
           <div class='box box-solid'>
                <div class='box-header with-border '>
                    <h3 class='box-title'><i class="fas fa-book fa-fw   "></i>Lemari Arsip Surat Keluar</h3>
                    <div class='box-tools pull-right'>
				...
				$bl=date('m');
				$th=date('Y');
                $guruQ = mysqli_query($koneksi, "SELECT * FROM surat_keluar WHERE bln='$bl' AND thn='$th'");
              ...
              $arsipQ = mysqli_query($koneksi, "SELECT * FROM surat_keluar WHERE kode='$guru[kode]' AND bln='$bl' AND thn='$th'");
			...
			$map = fetch($koneksi,'map',['id_map' =>$arsip['idmap']]);
                ...
                ?>
                                    <div class="box-footer no-padding">
										...
										<li>
                                                <a href="?pg=surat_keluar&ac=lihatarsip&ids=<?= $arsip['id_surat'] ?>" title="Detail Data">
                                                    <i class='fas fa-envelope'></i> Buka Arsip
                                                    <span class="pull-right badge bg-maroon"><?= $arsip['tujuan_surat'] ?></span>
                                                </a>
                                            </li>
  ...
  <?php $surat = fetch($koneksi,'surat_keluar',['id_surat' =>$_GET['ids']]); ?>
    <div class='row'>
        <div class='col-md-12'>
            <div class='box box-solid'>
                <div class='box-header with-border '>
                    <h3 class='box-title'> Tujuan Surat : <b><?= $surat['tujuan_surat'] ?></b></h3>
						  <div class='box-tools pull-right '>
                      ...
                      <div id='tabelarsip' class='table-responsive'>
                        <table id="tabelarsip" class='table table-bordered table-striped  table-hover'>
                            ...
                            <tbody>
							       <?php     
								$no=0;
                              $materiQ = mysqli_query($koneksi, "SELECT * FROM surat_keluar WHERE id_surat='$_GET[ids]'");
                              ...
                              $no++;
								...
								<tr>
									   ...
									   <?php if($dokumen['file']<>''){ ?>
									   <a href="mod_arsip/keluar.php?file=<?= $dokumen['file'] ?>"  class="btn btn-primary btn-sm"><i class="fas fa-download"></i></a>
									   <?php }else{ ?>
	...
	<?php
	   ...
	   move_uploaded_file($_FILES["gb1"]["tmp_name"],"../arsip/surat_keluar/".$_FILES["gb1"]["name"]);
	   
	  $exec = mysqli_query($koneksi, "INSERT INTO surat_keluar(idlemari,idmap,no_agenda,nomor_surat,tujuan_surat,isi,kode,tgl_surat,tgl_catat,file,user,bln,thn) VALUES
	...
	echo "
	  <script type='text/javascript'>
		...
		window.setTimeout(function(){ 
			window.location.replace('?pg=surat_keluar');
		} ,2000);	
	  </script>";
	...
	echo "
	  <script type='text/javascript'>
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: 'mod_arsip/hapus_keluar.php',
                    method: "POST",
                    data: 'id=' + id,
